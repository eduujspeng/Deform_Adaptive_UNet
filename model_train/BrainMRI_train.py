...
if __name__ == '__main__':
    ...
    for epoch in range(300):
        print('Unet Plus第{}轮训练开始'.format(epoch + 1))
        ...
        val_dl = DataLoader(val_data, batch_size=8)

        net.train()
        train_loss, train_dice, train_iou = model_train(net, train_dl, optimizer, criterion,deep=False)
        train_loss_list.append(train_loss)
        train_dice_list.append(train_dice)
        train_iou_list.append(train_iou)

        net.eval()
        val_loss, val_dice, val_iou = model_test(net, val_dl, criterion,deep=False)
        test_loss_list.append(val_loss)
        test_dice_list.append(val_dice)
        test_iou_list.append(val_iou)
        if val_dice > tmp and epoch > 250:
            tmp = val_dice
            torch.save(net.state_dict(), '../pth/Unet_plus_best.pth')  # model save
